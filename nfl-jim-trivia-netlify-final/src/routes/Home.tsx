...
export default function Home(){
  ...
  React.useEffect(()=>{ if(ds.ready && !g.round.active) g.startRound(ds) },[ds.ready])
  ...
  return (
    <div>
      ...
      {g.round.revealed && <div style={{marginTop:12}}><RevealCard /><div style={{marginTop:12}} className="row"><button className="btn" onClick={()=>g.startRound(ds)}>Next Round</button></div></div>}
    </div>
  )
}
